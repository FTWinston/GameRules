...
{
    public class CellReferences : SentenceParser<GameDefinitionBuilder>
    {
        ...
        public override string[] Examples => new[]
        {
            ...
            "Cells on the board are referenced using a number, starting with 1 at the southwest corner and changing direction each row",
        };

        // TODO: handle "snakes and ladders" winding
        protected override string ExpressionText => $"Cells on (?:the )?({WordExpression}) are referenced using a ({WordExpression})( for their {WordExpression})(?: and a ({WordExpression}) for their ({WordExpression}))?, starting (?:with|from) ({WordExpression}) at the ({WordExpression}) corner";
